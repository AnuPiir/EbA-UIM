...
export class ValidationComponent implements OnInit, AfterContentChecked {
  ...
  async addFeatureAfter(currentFeatureId: number, currentFeatureGroupId: number) {
    const currentGroupRows = this.validationRowValues.filter(row => row.answers[0].featureGroupId === currentFeatureGroupId);
    const currentFeatureRows = currentGroupRows.filter(row => row.answers[0].feature.id === currentFeatureId);

    if (currentFeatureRows.length === 0) { return; }

    const maxRowIdOfCurrentFeature = Math.max(...currentFeatureRows.map(row => row.rowId));
    const featuresToShift = currentGroupRows.filter(row => row.answers[0].feature.id > currentFeatureId);
