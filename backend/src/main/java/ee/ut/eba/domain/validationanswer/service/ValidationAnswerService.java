...
public class ValidationAnswerService {
	...
	public ValidationAnswer save(SaveParameters params) {
		...
		ValidationAnswer validationAnswer = new ValidationAnswer().setAnswer(params.answer()).setId(params.id())
				.setRowId(params.rowId()).setType(params.type())
	...
	public void delete(Integer questionnaireId, Integer rowId) {
		validationAnswerRepository
				.delete(ValidationAnswerSpecification.questionnaireId(questionnaireId).and(rowId(rowId)));
	}

	public void removeStakeHolderById(Integer id) {
		log.info("Removing stakeholder with id: {} from all validation answers", id);
		List<ValidationAnswer> validationAnswers = validationAnswerRepository
				.findAll(ValidationAnswerSpecification.stakeHolderId(id));
	...
	public void deleteByFeatureId(Integer id) {
		...
		validationAnswerRepository
				.deleteAll(validationAnswerRepository.findAll(ValidationAnswerSpecification.featureId(id)));
	}
	...
	public void deleteByFeaturePreconditionId(Integer id) {
		...
		validationAnswerRepository
				.deleteAll(validationAnswerRepository.findAll(ValidationAnswerSpecification.featurePreconditionId(id)));
	}

	public void updateStakeHolderAnswers(Integer stakeholderId, String stakeholderName) {
		List<ValidationAnswer> validationAnswers = validationAnswerRepository
				.findAll(ValidationAnswerSpecification.stakeHolderId(stakeholderId));
		for (ValidationAnswer validationAnswer : validationAnswers) {
			if (validationAnswer.getType().equals("STAKEHOLDER")) {
